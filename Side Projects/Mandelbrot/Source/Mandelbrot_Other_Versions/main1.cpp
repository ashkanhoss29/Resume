...
int init_resources(void)
{
  ...
  const char *fs_source =
    ...
    "void main(void) {                                                     "
	"  double x0 = ((double(gl_FragCoord.x) * (x_max - x_min)) / double(1920.0)) + x_min;  "
	"  double y0 = ((double(gl_FragCoord.y) * (y_max - y_min)) / double(1080.0)) + y_min;  "
	"  double hello = double(0.0);                                                  "
	"  double x = double(0.0); "
	"  double y = double(0.0);                                                      "
	"  double xtemp = double(0.0);                                                  "
	"  int iteration = 0;                                            "
	"  int max_iteration = Scale;                                      "
	"  while (x*x + y*y < double(1.0 << 16.0) && iteration < max_iteration)          "
	"  {                                                             "
	"     xtemp = x*x - y*y + x0;                                    "
	"     y = double(2.0)*x*y + y0;                                            "
	"     x = xtemp;                                                 "
	...
	"  }                                                             "
	"  float d = 1.0 - (float(iteration) / float(max_iteration)); "
