...
public class ThreadPanel extends JPanel {
    ...
    public void initializeCanvas()
    {
        ...
        if(runImage != null)
            run = new JButton(new ImageIcon(runImage));
        else
            run = new JButton("Run");
        p.add(run);
        run.setPreferredSize(new Dimension(32, 32));
    ...
    public void setRunPauseImages(Image tRun, Image tPause)
    {
        runImage = tRun;
        pauseImage = tPause;
    }

    public void setProgressImage(Image tLight)
    {
        canvas_.setProgressImage(tLight);
    }
...
// Change to show crash
    public void start(Runnable r) {
        thread_ = new DisplayThread(canvas_,r,5,hasSlider?bar_.getValue():0,true);
        thread_.start();
    }

    public Thread start(Runnable r, boolean suspended) {
        thread_ = new DisplayThread(canvas_,r,5,hasSlider?bar_.getValue():0,suspended);
        thread_.start();
        return thread_;
    }
...
class GraphicCanvas extends JPanel {
    ...
    GraphicCanvas(String title, Color c) {
        super();
        title_=title;
        setPreferredSize(new Dimension(139,200));
        arcColor_=c;
        progressImage = null;
  	}
    ...
    public synchronized void update(Graphics g){
	    ...
	    offgraphics.fillRect(0, 0, d.width, d.height);
         ...
         {
              offgraphics.drawImage(progressImage, 0, 0, this);
              //offgraphics.setColor(new Color(0, 256, 0, 120));
              offgraphics.setColor(arcColor_);
              offgraphics.fillOval(20, 55, 100, 100);

              // draw a partial image to cover the undesired areas
              float fRad = 100.0f;
              float fHeight = (360-angle_) / 360.0f * fRad;
              int iNewHeight = (int)fHeight;
              int iWidth = progressImage.getWidth(this);
              offgraphics.drawImage(progressImage, 20, 55, 178, iNewHeight+55, 20, 55, 178, iNewHeight+55, this);
         }
