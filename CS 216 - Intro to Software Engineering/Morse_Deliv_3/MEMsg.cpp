...
void MEMsg::send( string filename, const string& morse )
{
	...
	int numberOfBytes = morse.size()/4+2;
	unsigned char* bytesToWrite = new unsigned char[numberOfBytes];
	int byteCounter = 1;

	bytesToWrite[0] = numberOfBytes; //Store how many bytes we used
...
string MEMsg::receive( string filename )
{
	unsigned char bitMasks[4][3];
	...
	unsigned int numberOfBytes = (unsigned char)bytesToRead[0];
